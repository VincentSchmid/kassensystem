...
class QueryTestCases(TestCase):
    def test_get_order_total(self):
        ...
        OrderItem.objects.create(order=order, menu_item=MenuItem.objects.create(price=10, name="Pizza"), quantity=2)
        OrderItem.objects.create(order=order, menu_item=MenuItem.objects.create(price=20, name="Pasta"), quantity=1)
    ...
    def test_get_orders_with_order_items(self):
        ...
        self.assertEqual(get_orders_with_order_items().first().order_items.first().menu_item.price, 10)
        self.assertEqual(get_orders_with_order_items().first().order_items.last().menu_item.price, 20)
